...
app.post('/api/auth/sign-in', async (req, res, next) => {
  try {
    ...
    const userHashed: { userId: string; hashedPassword: string } =
      result.rows[0];
    const validPassword = await argon2.verify(
      userHashed.hashedPassword,
      password
    );
    if (!validPassword) {
      throw new ClientError(401, 'invalid login');
      ...
      const payload = {
        userId: userHashed.userId,
        username,
      };
      const secret =
        process.env
          .TOKEN_SECRET!; /* pretty certain this will never be undefined */
